...
public class MatrixMultiplicationBenchmark implements Benchmark{
	...
	public void runOnGPU(int arrayLen) {
		...
		Method m = null;
		try {
			m = MatrixMultiplicationBenchmark.class.getDeclaredMethod("matrixMultiply", int[][].class, int[][].class, int[][].class);
		...
		int[][] a = new int[arrayLen][arrayLen];
		int[][] b = new int[arrayLen][arrayLen];
		int[][] c = new int[arrayLen][arrayLen];
		...
		System.out.println("GPU Time: " + (endTime - startTime));
	}
	...
	public void runHandWritten(int arrayLen) {
		String kernelSource = "";
		try {
			kernelSource = Helpers.getKernel("src/MatrixMultiplication/matrix_multiplication.cl");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			return;
		}
		String kernelName = "matrix_multiplication";
		...
		int[] flatA = (int[]) ArrayUtil.transformArray(a);
		int[] flatB = (int[]) ArrayUtil.transformArray(b);
		int[] flatC = (int[]) ArrayUtil.transformArray(c);
        ...
        CL.clEnqueueReadBuffer(commandQueue, memObjects[2], CL.CL_TRUE, 0, Sizeof.cl_int * flatC.length, 
        		Pointer.to(flatC), 0, null, null);
        ...
        ArrayUtil.rebuild(flatC, c, new int[] {0});
